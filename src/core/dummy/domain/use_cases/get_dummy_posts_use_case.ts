...
import type { GetPostsInput, IDummyRepository } from "@/src/core/dummy/domain/interfaces/dummy_repository";
...
export class GetDummyPostsUseCase {
  ...
  async execute(input?: GetPostsInput): Promise<Array<DummyPost>> {
    const dummyRepository = await this.dummyRepositoryProvider();
    return dummyRepository.posts(input);
  }
}
